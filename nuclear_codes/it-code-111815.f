...
c      integer(2),allocatable::iloc(:,:)
      ...
      module nutbme
      integer ntbme
      integer Aeff ! SRS added for A-dependence study
      end module nutbme
...
c      integer(8) :: nhme
      ...
      module spb
      integer :: nucleons,nprotons,nneutrns,mnop,mnop2,igt,irest=0
...
c         dim=dim_st*3/2
         dim=2*(2**(nint(log(real(dim_st*dimfac,
     $        kind(0.d0)))/log(2.d0))))
      else
c         dim=dim_st
         dim=2147483648
...
c     if the added state is a proton or neutron state
     ...
     $     nwd2,index)
      use nodeinfo
      use parameters
      use spb, only : nwd, nprotons, nneutrns, nucleons
      use ibas
      implicit none
...
c     MKGK : called with nwd2 instead of nwd (as in Jplus)
      ...
      if (index>-1) then
         print *,' inside insert_hash: index=',index
         return
      endif
...
c** PN
            ...
            do ii=kappa_points,1,-1
               ...
               if (irest==4.and.nhw_boot==nhw_restart
     $              .and.kappa_restart<ii) then
                  cycle
               elseif (irest==4.and.nhw_boot==nhw_restart
     $                 .and.kappa_restart==ii) then
...
c     Read input from the file "mfdp.dat" and do some initializations:
      ...
      include 'mpif.h'
      character(len=120) :: outfile,v3intfile
c     mnpi(N): minimum number of protons  in the N-th shell
c     mnnu(N): minimum number of neutrons in the N-th shell
c     mxpi(N): maximum number of protons  in the N-th shell
c     mxnu(N): maximum number of neutrons in the N-th shell
...
c     ----- Read in min(N1 or N2), max(N1 or N2) and max(N1+N2),  here N=2n+l:
 ...
 93      format(3x,'G matrix triangle: min(N) =',i2,'   max(N) =',i2,
     &        '   max(N1+N2) =',i3)
         if(iham.eq.1) write(9,*) '  Use H = Trel + G.'
         if(iham.eq.2) write(9,*) '  Use H = Hrel + G.'
         if(iham.eq.3) write(9,*) '  Use H = SPE + G.'
         if(iclmb.eq.0)write(9,*) '  Without the Coulomb interaction.'
         if(iclmb.eq.1)write(9,*) '  With the Coulomb interaction.'
...
c     ----- to skip, and iset1 (search for "iset1" for its usage)
      ...
      read(1,*)
      read(1,*)irest            ! irest=4: Restarting;  irest=0: Initial run
...
C**   end of importance truncation variables
     ...
     $           mxn2l,N_12max,E1max,E12max
      ...
      read(1,*)Aeff  ! SRS effective mass for trel
      write(*,*)'Aeff = ',Aeff
...
c
     ...
     &     itrel,ihrel,ispe,nskip)
      ...
      use tbmes
      include 'mpif.h'
      character(len=80),intent(IN) :: intfile
...
c
      ...
      if(Aeff.eq.-1) Aeff=nucleons
c      comft = 2.*hbomeg/real(Aeff)   !SRS This should be changed nucleons -> Aeff
...
cSRS      comfcm2 = strcm*2.*hbomeg/real(nucleons) 
      comfcm1 = strcm*hbomeg/real(Aeff-1)
      comfcm2 = strcm*2.*hbomeg/real(Aeff)
...
c**********************************************
cSRS Here is where trel gets added to the tbme
         do jset = 0, nsetm1, 1
...
c     DRF Determine term to add
               if (dd.eq..true.) then
                  ...
                  if (laa.eq.lbb.and.jaa.eq.jbb) then
                     if (naa.eq.nbb) then
                        trelaeff = trelaeff +
     &                       pp*(2*naa+laa+1.5)/8.0
                     elseif (naa.eq.nbb+1) then
                        trelaeff = trelaeff +
     &                       pp*sqrt(naa*(naa+laa+0.5))/8.0
                     elseif (nbb.eq.naa+1) then
                        trelaeff = trelaeff +
     &                       pp*sqrt(nbb*(nbb+lbb+0.5))/8.0
                     endif
                  endif
               endif
            enddo
...
c$$$                  if (ia.eq.2.or.ib.eq.2.or.ic.eq.2.or.id.eq.2) then
c$$$                     print *,'  n2,l2,j2  = ',nnl(2),lx(2),j2x(2),'/2'
c$$$                  endif
c$$$                  if (ia.eq.3.or.ib.eq.3.or.ic.eq.3.or.id.eq.3)
c$$$     &                    then
c$$$                     print *,'  n3,l3,j3  = ',nnl(3),lx(3),j2x(3),'/2'
c$$$                  endif
c$$$                  print *,'  trel file = ',trel
c$$$                  print *,'  trel aeff = ',trelaeff
c$$$                  print *,'  comft     = ',comft
c$$$                  print *,'  comft2    = ',comft2
c$$$               endif
c$$$            endif
...
c     ----- First construct H + strcm*(H0-(N0+1.5)hw):
     ...
     &           -(2.0*mnn2l+3.0)/real(Aeff))
cSRS     &           -(2.0*mnn2l+3.0)/real(nucleons))
...
c      close(61)
     ...
     $        status='unknown',action='write')
      ...
      integer(4),allocatable :: iloc_tmp_buf1(:)
      integer :: ibuf,iallredbuf,iiy,nucl,iallred,ierr
      character(len=100) :: filenmtmp
...
c     test variables
      integer :: state_adder, state_adder_mpi
      integer :: state_deleter, state_deleter_mpi
...
c     ----- The "2" below indicates maximum of 2-body operators present
c**   mnop = 2
      if (mnop==3) then
         mnop2=6
      elseif (mnop==-3) then
...
c     if(irest/=2.and.irest/=4) then
c     if (iproc==0) then
c     mysave=mysave+1
c     write(12)(amp(i1,1),i1=1,nsd)
c     endif
c     endif   
c     
c     
...
c     
c     if (irest==4) then
c     open(29,file='lanczme.tmp',status='old')
c     iix=0
c     do ij=1,nnmax
c     read(29,*,end=4574,err=4574) iix,alpha(ij),beta(ij)
c     end do
c     4574    continue
c     c**         iterest=max(1,iix-nproc)
c     c**         iterest=max(1,iix-2)
ccc   iterest=iix
c     iterest=iix+1
c     if (iproc==0) then
c     print *,' iterest=',iterest
c     write(9,*) ' Restarting Lanczos iterations from'
c     +           ,' #',iterest
c     write(8,*) ' Restarting Lanczos iterations from'
c     +           ,' #',iterest
c     endif   
c     rewind(29)
c     if (iterest>1) then
c     if (iproc==0) then
c     read(12) (amp(i1,1),i1=1,nsd)
c     mysave=mysave+1
c     endif
c     endif   
c     endif   
...
c                  if (i3==-1) cycle
                  if (i3 > 0) cycle
                  state_adder_mpi = state_adder_mpi+1
...
c     $                 an_st_1,an_st_2,an_st_3)
     ...
     $                 an_st_1,an_st_2,an_st_3)
                  ...
                  IT_test=.false.
                  do k1=1,nf
                     if (dabs(xcoef*amp(i1,k1))/(2.d0*hbomeg)>kappa)then
                        IT_test=.true.
                        exit
                     endif
                  end do
...
c      N12_max = N12_max_hold
     ...
     $     nsdmaxtmp(3),stat,ise,ip,in,tag3,tag4
...
c            nsdnf=nsdmax(3)
     ...
     $              icomm,ierr)
      ...
      integer,allocatable :: iloc_tmp(:,:)
      real(kind(0.d0)),allocatable :: bmp_orig(:,:),bmp_new(:,:)
      integer :: exminho,Nmin_HO
      ...
      if (iproc==0) then
         ...
         if (piv_saved) then
            allocate(bmp_orig(nsd_orig,0:nf))
            allocate(bmp_new(nsd,0:nf))
            bmp_new=0.d0
            do i1=0,nf
               call read_pivot(nsd_orig,bmp_orig(:,i1),i1) 
            end do
         endif
      endif
      ...
      do N=nminho,nhw,2
         do i1=1,nsd
            ...
            if (N==NHO_i1) then
               ...
               if (iproc==0.and.i1<=nsd_orig) then
c                  if (i1/=i_count) then
c                     print *,' N,i1,i_count=',N,i1,i_count
c                  endif
                  do i2=0,nf
                     bmp_new(i_count,i2)=bmp_orig(i1,i2)
                  end do
               endif
            endif
         end do
     ...
     +           form='unformatted',status='unknown')
            write(30) (nsd)
            write(30) (bmp_new(i1,0),i1=1,nsd)
            close(30)
            print *,' piv.tmp saved'
            do i1=1,nf
               open(30,file='piv'//achar(i1-1+48)//'.tmp',
     +              access='sequential',
     $              form='unformatted',status='unknown')
               write(30) (nsd)
               write(30) (bmp_new(i2,i1),i2=1,nsd)
               close(30)
               print *,' piv'//achar(i1-1+48)//'.tmp saved'
            end do
            deallocate(bmp_new)
         endif
         print *, '--iloc and pivot reordered: nsd_orig,nsd,i_count=',
...
c                                 print *,' i3=',i3
     ...
     $                          *ham3b_cJ(cr_st_1,cr_st_2,cr_st_3,
     $                          an_st_1,an_st_2,an_st_3)
...
************************************************************
                     ...
                     if (N_HO_f<N_HO_i) amp(i1,nb) = amp(i1,nb)
     $                          +xcoef*amp(i3,na)
c                           if(i1.ne.i3) amp(i1,nb) = amp(i1,nb)
c     $                          +xcoef*amp(i3,na)
                  end do
               end do
            endif
...
c     
                        call tbmeop(rmop1,xj2op1,t2op1,xgtp,xgtm)
                        xrm = phti * rmop1
                        xj2 = phti * xj2op1
                        xt2 = phti * t2op1
                        gtp = phti * xgtp
...
c    &           (real(jx21+1)))*t1bqsm**2
     ...
     &           '  neutron=',t1bqnsm(k1),' B(E2)=',t1bqsm
         ...
         inquire(unit=675,opened=kappadepwr)
         if (kappadepwr) then
            write(675,'(e12.5,1x,f10.4,1x,f8.4,1x,f8.4,i4)') kappa,
     $           xcoefs(k1),xj,xt,kfx
         endif
         write(9,*)
...
c     ----- rmop, xj2op, t2op etc.
      subroutine tbmeop(rmox,xj2ox,t2ox,xgtp,xgtm)
      use parameters
      use jsgna
      use consts
      use spb
