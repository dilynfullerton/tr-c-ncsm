...
if __name__ == "__main__":
    ...
    if not exact_range:
        ...
        if len(user_args) == 2:
            ...
            ncsd_exact_calculations(z=z0, a_range=[amin], force=force0)
        elif len(user_args) == 3:
            z0, amin, amax = [int(x) for x in user_args]
            ncsd_exact_calculations(z=z0, a_range=range(amin, amax+1),
                                    force=force0)
        elif len(user_args) == 4:
            ...
            ncsd_exact_calculations(z=z0, a_range=range(amin, amax+1),
                                    n1=n1_, n2=n2_, force=force0)
        elif len(user_args) == 6:
            z0, amin, amax, nhw0, n1_, n2_ = [int(x) for x in user_args]
            ncsd_exact_calculations(z=z0, a_range=range(amin, amax+1),
                                    nhw=nhw0, n1=n1_, n2=n2_, force=force0)
        else:
            raise InvalidNumberOfArgumentsException(
                '%d' % len(user_args) +
                ' is not a valid number of arguments for ncsm_single_calc.py.' +
