...
def _make_base_directories(z_a_aeff_to_dpath_map):
    """Makes directories for (Z, A, Aeff) pairs if they do not exist yet
    :param z_a_aeff_to_dpath_map: map from (Z, A, Aeff) to NCSD directories
    """
    for d in z_a_aeff_to_dpath_map.values():
...
def _make_mfdp_files(
        z_list, a_list, aeff_list, nhw_list, nshell, n_1, n_2,
        beta_cm, num_states, num_iter, z_a_aeff_to_dpath_map,
        z_a_aeff_to_outfile_fpath_map, fname_tbme, dpath_temp,
):
    """Writes mfdp files for the specified NCSD calculations
    :param z_list: ordered list of Z values
    ...
    :param z_a_aeff_to_dpath_map: map from (A,Aeff) to the directory in which
    the NCSD calculation was done
    :param z_a_aeff_to_outfile_fpath_map: map from (A,Aeff) to the *.out file
    to be generated by NCSD
    :param fname_tbme: name of the TBME interaction file
    :param dpath_temp: path to the templates directory in which the mfdp.dat
    template file is stored
    """
    for z, a, aeff, nhw in zip(z_list, a_list, aeff_list, nhw_list):
        outfile_name = path.split(
            z_a_aeff_to_outfile_fpath_map[(z, a, aeff)])[1]
        _make_mfdp_file(
            z=z, a=a, aeff=aeff, nhw=nhw, nshell=nshell, n1=n_1, n2=n_2,
            beta_cm=beta_cm, num_states=num_states, num_iter=num_iter,
            dpath_elt=z_a_aeff_to_dpath_map[(z, a, aeff)],
            dpath_temp=dpath_temp, fname_outfile=outfile_name,
...
def _get_trdens_replace_map(a, a0, nshell, z):
    """Given a mass number, the first mass number in the shell, and the shell,
    ...
    :param z: proton number
    """
    nnn, num_states = _get_num_states(a, a0, nshell, z)
    return {'<<NNN>>': str(nnn), '<<NUMSTATES>>': str(num_states)}


def make_trdens_file(
        a, a0, nuc_dir, nshell, z, dpath_results=DPATH_RESULTS,
    ...
    rep_map = _get_trdens_replace_map(a=a, a0=a0, nshell=nshell, z=z)
...
def _make_job_submit_files(
        job_files, walltime, use_mpi, fname_tmp_jobsub
):
    """For each job file in job_files, makes the job.sh
    ...
    :param job_files: list of job submit files
...
def get_z_a_aeff_to_dpath_map(
        z_list, a_list, aeff_list, nhw_list, n1, n2, scalefactor=None,
        remove_protons=False, dpath_results=DPATH_RESULTS,
        dname_ncsd=DNAME_NCSD,
):
    """Given ordered lists z_list, a_list, aeff_list, nhw_list, 
    constructs a map from (Z, A, Aeff) to the directory in which the 
    NCSD calculation is to be done
    :param z_list: ordered list of proton numbers (Z)
...
def get_z_a_aeff_to_outfile_fpath_map(
        z_list, a_list, aeff_list, nhw_list, n1, n2, remove_protons,
        z_a_aeff_to_dirpath_map, scalefactor=None
):
    """Given ordered lists z_list, a_list, aeff_list, nhw_list, 
    constructs a map from (Z, A, Aeff) to the NCSD *.out file that will 
    be created
    :param z_list: ordered list of proton numbers (Z)
    :param a_list: ordered list of mass numbers (A)
    :param aeff_list: ordered list of effective mass numbers (Aeff),
    corresponding to the mass numbers in a_list
    :param nhw_list: ordered list of Nhw corresponding to the mass 
    numbers in a_list
    :param n1: max allowed one-particle state (for TBME truncation)
    :param n2: max allowed two-particle state (for TBME truncation)
    :param z_a_aeff_to_dirpath_map: map from (Z, A, Aeff) to the 
    directory in which an NCSD calculation will be done
    :param scalefactor: value by which off-diagonal coupling terms in the
    TBME interaction were scaled.
    :param remove_protons: either true or false, indicating whether the
    proton parts of the interaction (Vpp and Vpn) are being scaled to 0
    """
    z_a_aeff_outfile_map = dict()
    ...
    for z, a, aeff, nhw in zip(z_list, a_list, aeff_list, nhw_list):
        ...
        if scalefactor is not None:
            args += (scalefactor,)
        z_a_aeff_outfile_map[(z, a, aeff)] = path.join(
            z_a_aeff_to_dirpath_map[(z, a, aeff)], fname_fmt % args)
    return z_a_aeff_outfile_map


def _get_z_a_aeff_to_egv_fpath_map(
        z_list, a_list, aeff_list, nhw_list, z_a_aeff_to_dirpath_map):
    """Given ordered lists z_list, a_list, aeff_list, nhw_list, 
    constructs a map from (Z, A, Aeff) to the *.egv file, created by 
    NCSD, which is used to signify that the calculation has been completed
    :param z_list: ordered list of proton numbers (Z)
    :param a_list: ordered list of mass numbers (A)
    :param aeff_list: ordered list of effective mass numbers (Aeff),
    corresponding to the mass numbers in a_list
    ...
    directory in which an NCSD calculation will be done
    """
    z_a_aeff_to_egv_map = dict()
    for z, a, aeff, nhw in zip(z_list, a_list, aeff_list, nhw_list):
        z_a_aeff_to_egv_map[(z, a, aeff)] = path.join(
            z_a_aeff_to_dirpath_map[(z, a, aeff)], FNAME_FMT_EGV % nhw)
    return z_a_aeff_to_egv_map


def _get_z_a_aeff_to_jobsub_fpath_map(
        z_list, a_list, aeff_list, nhw_list, n1, n2, remove_protons,
        z_a_aeff_to_dirpath_map, scalefactor=None,
):
    """Given ordered lists z_list, a_list, aeff_list, nhw_list, 
    constructs a map from (Z, A, Aeff) to the *.sh file that will be 
    ...
    for z, a, aeff, nhw in zip(z_list, a_list, aeff_list, nhw_list):
        ...
        if scalefactor is not None:
            args += (scalefactor,)
        z_a_aeff_jobsub_map[(z, a, aeff)] = path.join(
            z_a_aeff_to_dirpath_map[(z, a, aeff)], fname_fmt % args)
    z_a_aeff_to_ncsd = dict()
    z_a_aeff_to_vce = dict()
    for k, v in z_a_aeff_jobsub_map.items():
        z_a_aeff_to_ncsd[k] = v + '_NCSD'
        z_a_aeff_to_vce[k] = v + '_TRDENS'
    return z_a_aeff_to_ncsd, z_a_aeff_to_vce
...
def prepare_directories(
        z_list, a_list, aeff_list, nhw_list, n1, n2, nshell, scalefactor,
    ...
    :param z_list: ordered list of Z values for which to create directories
    ...
    """
    # get maps
    z_a_aeff_to_dir_map = get_z_a_aeff_to_dpath_map(
        z_list=z_list, a_list=a_list, aeff_list=aeff_list, nhw_list=nhw_list,
        n1=n1, n2=n2, scalefactor=scalefactor, remove_protons=remove_protons,
        dpath_results=dpath_results,
    )
    z_a_aeff_to_outfile_map = get_z_a_aeff_to_outfile_fpath_map(
        z_list=z_list, a_list=a_list, aeff_list=aeff_list, nhw_list=nhw_list,
        n1=n1, n2=n2, z_a_aeff_to_dirpath_map=z_a_aeff_to_dir_map,
        scalefactor=scalefactor, remove_protons=remove_protons,
    )
    z_a_aeff_to_egvfile_map = _get_z_a_aeff_to_egv_fpath_map(
        z_list=z_list, a_list=a_list, aeff_list=aeff_list, nhw_list=nhw_list,
        z_a_aeff_to_dirpath_map=z_a_aeff_to_dir_map
    )
    if cluster_submit:
        ncsd_map, vce_map = _get_z_a_aeff_to_jobsub_fpath_map(
            z_list=z_list, a_list=a_list, aeff_list=aeff_list,
            nhw_list=nhw_list, n1=n1, n2=n2,
            z_a_aeff_to_dirpath_map=z_a_aeff_to_dir_map,
            scalefactor=scalefactor, remove_protons=remove_protons,
        )
    else:
    ...
    _make_base_directories(z_a_aeff_to_dpath_map=z_a_aeff_to_dir_map)
    ...
    fname_tbme, lname_tbme = _truncate_spaces(
        nshell=nshell, n1=n1, n2=n2, dirpaths=z_a_aeff_to_dir_map.values(),
    ...
    _make_mfdp_files(
        z_list=z_list, a_list=a_list, aeff_list=aeff_list, nhw_list=nhw_list,
        z_a_aeff_to_dpath_map=z_a_aeff_to_dir_map,
        z_a_aeff_to_outfile_fpath_map=z_a_aeff_to_outfile_map,
        fname_tbme=lname_tbme, dpath_temp=dpath_templates, nshell=nshell,
    ...
    if cluster_submit:
        ...
        _make_job_submit_files(
            job_files=ncsd_map.values(), walltime=walltime,
            use_mpi=use_mpi, fname_tmp_jobsub=FNAME_TMP_JOBSUB_NCSD
        )
        _make_job_submit_files(
            job_files=vce_map.values(), walltime=walltime,
            use_mpi=False, fname_tmp_jobsub=FNAME_TMP_JOBSUB_TRDENS
        )
    return (z_a_aeff_to_dir_map, z_a_aeff_to_egvfile_map, ncsd_map,
            vce_map, z_a_aeff_to_outfile_map)
